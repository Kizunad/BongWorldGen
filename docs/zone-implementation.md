...
第二十二步：区域权重查询按影响范围裁剪。`ZoneIndex` 预计算包含过渡带外半幅的
保守包围框，旋转裂谷使用旋转后的矩形外包，圆形取短轴直径。完全不相交的区域
直接跳过；部分相交时只计算可能非零的坐标，完整覆盖保留连续数组路径。
轮廓、权重公式、区域优先级和浮点乘加顺序保持不变。

全部形状、四种边界组合（含零宽）、细长旋转轮廓、负分数坐标、标量、空数组及
真实区域重叠都与未裁剪距离计算逐点相同。世界 401×409 网格的轮廓采样量从
**4,428,243** 降到 **16,791**，同机三次中位时间 1.548→0.100 秒；出生区 256×256
tile 从 27 次轮廓计算降为 1 次，背景 tile 降为 0。背景及所有区域权重逐字节相同。
记录：`docs/evidence/zone-query-culling-2026-09-22.json`。

```bash
.venv/bin/python tools/benchmark_zone_query.py
```
...
第三十步（2026-09-26 恢复）：区分洞穴与深渊的地表形态。普通洞穴加入三个分散的
陷穴，坑间保留较高地面；实际主门或回退入口另有局部塌陷和短沟。深渊使用现有主门
和坠渊井坐标构造折线低槽，两侧崖肩高低不同且终止位置错开。依据
`worldview.md §十三 L1267` 的地下网络、`worldview.md §十六 L1409` 的洞穴／崩石
入口描述及本库 POI 布局；没有把地表低槽视为地下秘境本体。所有变化位于合成层，
engine 不增加区域知识，洞穴层数、深度配方、基础高度和噪声参数保持原值。

新增 17 项契约，跨三 seed 覆盖两处普通洞穴的分散陷穴、坑间高地、真实入口与
岩盖，以及深渊的折线横断面、不等高崖肩和断口；移动 POI 后地表塌陷和实际竖井
共同移动。完整回归 **215 passed**，包括原三层壁片、井口连通和兴趣点支撑面断言。

seed=812731，与恢复前 `d76fd11` 的配方比较，三个陷穴的「周围八点中位高度－坑底」
在幽暗地穴由 **1.00/-0.07/-0.54** 变为 **16.96/17.15/17.29** 格，在暴龙王巢穴由
**-1.47/-0.55/-0.21** 变为 **14.73/16.54/17.07** 格；深渊两个横断面的「较低侧崖肩－槽底」
由 **-1.12/-1.98** 变为 **29.28/26.77** 格。验证空间结构，而非仅用均值或方差区分。

三个 seed（7、812731、2026）的全路径地下扫描，全部 **33/33** 次 POI 落点都能从
入口通过双格净空空气连通到达，三层结构没有因地表变化断开；本检查不代表可步行、
可攀爬或已满足游戏解锁规则。完整记录与数组校验值见
`docs/evidence/zone-cave-morphology-2026-09-26.json`，按下列命令替换 seed 即可复现。
